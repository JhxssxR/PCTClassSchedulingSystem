...
function reports_fetch_count(PDO $conn, string $sql, array $params = []): int {
    $stmt = $conn->prepare($sql);
    $stmt->execute($params);
    return (int)$stmt->fetchColumn();
}

function reports_day_short(string $day): ?string {
    $map = [
        'monday' => 'Mon', 'mon' => 'Mon',
...
function reports_initials(string $name): string {
    $name = trim($name);
    if ($name === '') {
        return 'NA';
    }
...
function reports_relative_time(string $datetime): string {
...
function reports_sparkline_points(array $values, int $width = 120, int $height = 34, int $padding = 3): string {
    $count = count($values);
    if ($count <= 1) {
        $y = $height - $padding;
        return "0,{$y} {$width},{$y}";
...
function reports_range_start(string $range): ?string {
    switch ($range) {
        case 'week':
...
$range_start = reports_range_start($selected_range);
...
try {
    $status_map = reports_enrollment_status_map($conn);
    $active_status = $status_map['active'];

    $enrollment_cols = reports_table_columns($conn, 'enrollments');
    $schedule_cols = reports_table_columns($conn, 'schedules');
    $classroom_cols = reports_table_columns($conn, 'classrooms');
    ...
    $total_courses = reports_fetch_count($conn, 'SELECT COUNT(*) FROM courses');
    $total_classrooms = reports_fetch_count($conn, 'SELECT COUNT(*) FROM classrooms');
    $total_instructors = reports_fetch_count($conn, "SELECT COUNT(*) FROM users WHERE role = 'instructor'");
    $total_schedules = reports_fetch_count($conn, 'SELECT COUNT(*) FROM schedules' . $range_schedule_where, $range_schedule_params);
    ...
    if ($has_schedule_status) {
        ...
        $active_classes = reports_fetch_count($conn, $active_sql, $active_params);
    } else {
    ...
    error_log('Reports page error (admin): ' . $e->getMessage());
}
...
foreach ($daily_stats as $row) {
    $short = reports_day_short((string)($row['day_of_week'] ?? ''));
...
foreach ($instructor_stats as $idx => &$instructor) {
    ...
    $instructor['initials'] = reports_initials($name);
...
$card_sparklines = [
    reports_sparkline_points($day_students),
    reports_sparkline_points($day_classes),
    reports_sparkline_points($room_series),
    reports_sparkline_points($avg_size_series),
];
...
if (!empty($_SESSION['first_name']) || !empty($_SESSION['last_name'])) {
    ...
    $user_initials = reports_initials($full_name);
}

if ((string)($_GET['export'] ?? '') === 'csv') {
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="admin-reports-' . date('Ymd-His') . '.csv"');

    $out = fopen('php://output', 'w');
    if ($out !== false) {
        fputcsv($out, ['Reports - Super Admin']);
...
<div class="min-h-screen report-surface">
    <aside id="sidebar" class="sidebar-shell fixed inset-y-0 left-0 z-40 -translate-x-full lg:translate-x-0 transition-transform bg-emerald-950 text-emerald-50 border-r border-emerald-900/40">
        ...
        <div class="px-4 py-4">
            ...
            <nav class="space-y-1">
                <a href="dashboard.php" class="sidebar-nav-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-emerald-50/80 hover:text-emerald-50 hover:bg-emerald-900/30"><i class="bi bi-speedometer2"></i><span class="text-sm font-medium">Dashboard</span></a>
                ...
                <a href="reports.php" class="sidebar-nav-link flex items-center gap-3 px-3 py-2.5 rounded-xl bg-emerald-900/40 text-emerald-50"><i class="bi bi-file-earmark-text"></i><span class="text-sm font-medium">Reports</span></a>
                ...
                <a href="../activity.php" class="sidebar-nav-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-emerald-50/80 hover:text-emerald-50 hover:bg-emerald-900/30"><i class="bi bi-clock-history"></i><span class="text-sm font-medium">Activity</span></a>
            </nav>
        </div>
    ...
    <div id="contentWrap" class="min-h-screen transition-all duration-300">
        ...
        <main class="px-4 sm:px-6 py-6">
            ...
            <section class="mt-4 grid grid-cols-1 xl:grid-cols-4 gap-4">
                    ...
                    <div class="mt-4 space-y-3">
                        <?php foreach ($recent_activity as $activity): ?>
                            ...
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <div class="text-[11px] text-slate-400 whitespace-nowrap"><?php echo htmlspecialchars(reports_relative_time((string)($activity['activity_at'] ?? ''))); ?></div>
                            </div>
                        <?php endforeach; ?>
...
(function () {
    ...
    sidebarBtn && sidebarBtn.addEventListener('click', function () {
...
(function () {
    ...
    markBtn && markBtn.addEventListener('click', async function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    delBtn && delBtn.addEventListener('click', async function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    scrollUpBtn && scrollUpBtn.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        notifList && notifList.scrollBy({ top: -scrollStep, behavior: 'smooth' });
    });

    scrollDownBtn && scrollDownBtn.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        notifList && notifList.scrollBy({ top: scrollStep, behavior: 'smooth' });
    });

    notifList && notifList.addEventListener('scroll', updateScrollButtons);
    updateScrollButtons();
