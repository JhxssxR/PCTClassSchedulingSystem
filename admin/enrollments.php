...
pgsql_fix_serial_sequence($conn, 'enrollments');
...
function display_status(string $db_status, string $active_db_status): string {
    $s = strtolower(trim($db_status));
    if ($s === strtolower($active_db_status)) {
        return 'active';
    }
    // UI groups rejected rows with dropped to match the design.
    if ($s === 'rejected') {
        return 'dropped';
    }
    return $s;
...
// Summary counts, normalized to UI buckets.
...
foreach ($counts_stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    $disp = display_status((string)($row['status'] ?? ''), $active_db_status);
    if (isset($status_counts[$disp])) {
        $status_counts[$disp] += (int)$row['c'];
    }
}
...
<meta charset="UTF-8">
    ...
    <title>Enrollments - PCT Class Scheduling</title>
    ...
    <div id="contentWrap" class="min-h-screen transition-all duration-300">
            ...
            <div class="h-full px-4 sm:px-6 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <span class="font-semibold text-slate-900">Enrollments</span>
                    </div>
                </div>
        ...
        <main class="px-4 sm:px-6 py-6">
            <?php
                ...
                $export_url = 'enrollments.php?' . http_build_query($export_params);

                $pagination_url = function (int $target_page) use ($status_filter, $q): string {
                    $params = ['page' => $target_page];
                    if ($status_filter !== 'all') $params['status'] = $status_filter;
                    if ($q !== '') $params['q'] = $q;
                    return 'enrollments.php?' . http_build_query($params);
                };

                $status_urls = [
                    'all' => 'enrollments.php?' . http_build_query(array_filter(['status' => 'all', 'q' => $q], fn($v) => $v !== '')),
                    'active' => 'enrollments.php?' . http_build_query(array_filter(['status' => 'active', 'q' => $q], fn($v) => $v !== '')),
                    'pending' => 'enrollments.php?' . http_build_query(array_filter(['status' => $supports_pending ? 'pending' : 'all', 'q' => $q], fn($v) => $v !== '')),
                    'dropped' => 'enrollments.php?' . http_build_query(array_filter(['status' => ($supports_dropped || $supports_rejected) ? 'dropped' : 'all', 'q' => $q], fn($v) => $v !== '')),
                ];
            ?>
                        ...
                        <div class="flex flex-wrap items-center gap-2 xl:justify-end xl:flex-none">
                            ...
                            <?php foreach ($chips as $k => $label): ?>
                                <?php
                                    ...
                                    $href = 'enrollments.php?status=' . urlencode($k);
                                    if ($q !== '') {
                                        $href .= '&q=' . urlencode($q);
                                    }
                                ?>
                        ...
                        <?php if ($has_schedule_options): ?>
                            <select id="addEnrollmentSubject" name="schedule_id" data-has-schedules="1" class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm" required>
                                ...
                                <?php foreach ($schedules as $schedule): ?>
                                    <option value="<?php echo (int)$schedule['id']; ?>" data-course="<?php echo (int)($schedule['course_id'] ?? 0); ?>">
                                        <?php
