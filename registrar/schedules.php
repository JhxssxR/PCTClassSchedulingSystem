            ...
            <tbody>
                <?php foreach ($schedules as $s): ?>
                    <?php $status = strtolower((string)($s['status'] ?? 'active')); ?>
                                <tr class="schedule-row border-t border-slate-100 hover:bg-slate-50/60" data-search="<?php echo htmlspecialchars(strtolower(($s['subject_code'] ?? '') . ' ' . ($s['subject_name'] ?? '') . ' ' . ($s['instructor_name'] ?? '') . ' ' . ($s['room_number'] ?? '') . ' ' . ($s['day_of_week'] ?? ''))); ?>">
                        <td class="px-5 py-4">
                                        <div class="font-semibold text-slate-900"><?php echo htmlspecialchars($s['subject_name'] ?? ''); ?></div>
                                        <div class="text-xs text-slate-500"><?php echo htmlspecialchars($s['subject_code'] ?? ''); ?></div>
                        </td>
                        ...
                        <td class="px-5 py-4">
                                        <span class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700"><?php echo htmlspecialchars($s['subject_code'] ?? ''); ?></span>
                        </td>
                        <td class="px-5 py-4">
