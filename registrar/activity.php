...
$total_items = count($all_activity_items);
$total_pages = max(1, (int)ceil($total_items / $per_page));
if ($current_page > $total_pages) {
    $current_page = $total_pages;
}
$offset = ($current_page - 1) * $per_page;
$page_activity_items = array_slice($all_activity_items, $offset, $per_page);
        ...
        <main class="flex-1 lg:ml-[290px]">
            ...
            <div class="px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
                    ...
                    <div class="relative p-6 sm:p-8 lg:p-10">
                        <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                            ...
                            <div class="grid gap-3 sm:grid-cols-3 lg:min-w-[420px]">
                                <div class="hover-card rounded-3xl border border-white/10 bg-white/10 p-4 backdrop-blur hover:border-white/20 hover:bg-white/15 hover:shadow-[0_18px_40px_rgba(15,23,42,0.18)]">
                ...
                <section class="mt-6 grid gap-4 md:grid-cols-3">
                    <div class="hover-card glass rounded-[24px] border border-emerald-900/10 p-5 shadow-sm hover:border-emerald-900/20 hover:shadow-[0_18px_40px_rgba(15,23,42,0.12)]">
                        <div class="flex items-center gap-3">
                ...
                <section class="mt-6 overflow-hidden rounded-[28px] border border-emerald-900/10 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
                    ...
                    <div class="divide-y divide-slate-100">
                        <?php if (!empty($page_activity_items)): ?>
                            <?php foreach ($page_activity_items as $item): ?>
                                <?php
                                    $event = (string)($item['event'] ?? '');
                                ...
                                ?>
                                <article class="hover-card flex flex-col gap-4 px-5 py-5 sm:px-6 lg:flex-row lg:items-start lg:justify-between hover:bg-emerald-50/30">
                                    ...
                                    <div class="shrink-0 rounded-2xl bg-slate-50 px-4 py-3 text-right ring-1 ring-inset ring-slate-100">
                                        <div class="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Timestamp (PH Time)</div>
                                        <div class="mt-1 text-sm font-semibold text-slate-800"><?php echo htmlspecialchars($when !== '' ? activity_format_timestamp_ph($when) : 'N/A'); ?></div>
                                    </div>
                                </article>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="hover-card px-5 py-16 text-center sm:px-6 hover:bg-emerald-50/20">
                    ...
                    <?php if ($total_pages > 1): ?>
                        <div class="flex flex-col gap-3 border-t border-slate-100 px-5 py-4 sm:px-6 sm:flex-row sm:items-center sm:justify-between">
                            <div class="text-sm text-slate-500">
                                Showing <?php echo number_format($offset + 1); ?> to <?php echo number_format(min($offset + $per_page, $total_items)); ?> of <?php echo number_format($total_items); ?> entries
                            </div>
                            <div class="flex items-center gap-2">
                                <a href="?page=<?php echo max(1, $current_page - 1); ?>" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 <?php echo $current_page <= 1 ? 'pointer-events-none opacity-40' : 'hover:bg-slate-50'; ?>">
                                    <i class="bi bi-chevron-left"></i>
                                    Prev
                                </a>
                                <div class="inline-flex items-center rounded-xl bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-700 ring-1 ring-inset ring-emerald-100">
                                    Page <?php echo number_format($current_page); ?> of <?php echo number_format($total_pages); ?>
                                </div>
                                <a href="?page=<?php echo min($total_pages, $current_page + 1); ?>" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 <?php echo $current_page >= $total_pages ? 'pointer-events-none opacity-40' : 'hover:bg-slate-50'; ?>">
                                    Next
